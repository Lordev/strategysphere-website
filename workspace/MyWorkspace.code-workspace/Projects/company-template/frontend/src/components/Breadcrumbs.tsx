            ...
            <ul className={`${containerClasses}`}>
                ...
                {finalPathNames
                    .map((link, index) => {
                        ...
                        const itemClasses = isLastItem ? `${activeClasses}` : listClasses;
                        const itemLink = capitalizeLinks
                            ? link[0].toUpperCase() + link.slice(1).replaceAll("-", " ")
                            : link.replaceAll("-", " ");
                    ...
                    .map(({ href, itemClasses, itemLink, isLastItem }, index) => (
                        <React.Fragment key={index}>
                            <li className={`${sharedClasses + itemClasses}`}>
                                {!isLastItem ? (
